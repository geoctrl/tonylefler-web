...
return (
  <Checkbox label="Accept terms and conditions Accept terms and conditions Accept terms and conditions Accept terms and conditions Accept terms and conditions Accept terms and conditions Accept terms and conditions Accept terms and conditions Accept terms and conditions Accept terms and conditions " checked={checked} onChangeValue={setChecked} />
)}
...
<PropsTable props={[
  ...
  },
  {
    name: "indeterminate",
    type: "boolean",
    description: "Displays the checkbox in an indeterminate (mixed) state, overrides checked appearance",
    required: false,
  },
]} />
...
Used when a checkbox represents a group where some (but not all) items are selected. Sets the native `indeterminate` DOM property on the input.

<Example>
  <div className="flex flex-col gap-3">
    <Checkbox label="Select all" indeterminate />
    <Checkbox label="Select all (disabled)" indeterminate disabled />
  </div>
</Example>
...
A common pattern is a "select all" checkbox that reflects mixed selection state:

export function IndeterminateExample() {
  const [items, setItems] = useState([
    { label: "First item", checked: true },
    { label: "Second item", checked: false },
    { label: "Third item", checked: true },
  ]);
  ...
  return (
    <div className="flex flex-col gap-2">
      <Checkbox
        ...
        onChangeValue={(val) => setItems(items.map(i => ({ ...i, checked: val })))}
      />
      <div className="ml-6 flex flex-col gap-2 border-l border-grey-200 dark:border-grey-700 pl-4">
        {items.map((item, idx) => (
          <Checkbox
            key={idx}
            label={item.label}
            checked={item.checked}
            onChangeValue={(val) => setItems(items.map((i, j) => j === idx ? { ...i, checked: val } : i))}
          />
        ))}
      </div>
    </div>
...
Validation errors are displayed as a danger badge below the checkbox.
...
- Error messages are displayed as a badge and announced to screen readers
- The `indeterminate` prop sets the native DOM `indeterminate` property, which screen readers announce as "mixed"
